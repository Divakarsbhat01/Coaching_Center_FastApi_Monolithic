...
def create_teacher(new_course:schemas.New_Teacher,db:Session=Depends(mysql_database_Config.get_db)
...
def getting_all_teachers(db:Session=Depends(mysql_database_Config.get_db),current_user:int=Depends(authentication.get_current_user)):
...
def deleting_teacher(id:int,db:Session=Depends(mysql_database_Config.get_db),current_user:int=Depends(authentication.get_current_user)):
...
def updating_teacher(id:int,update_teacher:schemas.Update_Teacher,db:Session=Depends(mysql_database_Config.get_db)
                     ,current_user:int=Depends(authentication.get_current_user)):
    x=db.query(models.Teacher).filter(models.Teacher.teacher_id==id)
    if x==None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Id does not exist")
